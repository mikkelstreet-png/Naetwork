...
export default function DashboardPage() {
    ...
    (async () => {
      ...
      if (prof?.user_type === 'professional' && pro) {
        const { data: bks } = await supabase
          .from('bookings')
          .select('*, professionals(name, title)')
  ...
  const totalEarnings = bookings
    ...
    .reduce((s, b) => s + (b.professional_payout_dkk as number), 0);
  const totalDonated = bookings
    .filter(b => b.status === 'completed' && b.donates_to_charity)
    .reduce((s, b) => s + (b.platform_fee_dkk as number), 0);
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="flex items-start justify-between mb-10">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              {isProfessional ? 'Mine sessioner' : 'Mine bookinger'}
            </h1>
            <p className="text-gray-500 mt-1">Hej, {profile?.name as string ?? user.email as string}</p>
          </div>
          {isProfessional && professional && (
            <Link href={`/professionals/${professional.id}`}
              className="text-sm text-green-800 border border-green-200 px-4 py-2 rounded-xl hover:border-green-800 transition-colors">
              Se min profil →
            </Link>
          )}
        </div>
            ...
            {professional?.donates_to_charity && (
              <div className="border border-rose-200 bg-rose-50 rounded-2xl p-5">
                <div className="flex items-center gap-1.5 text-xs text-rose-600 mb-1">
                  ...
                  Doneret til Kraeftens Bekaempelse
                </div>
        ...
        {bookings.length === 0 ? (
          <div className="text-center py-16 border border-gray-100 rounded-2xl">
            ...
            {!isProfessional && (
              <Link href="/professionals" className="text-sm text-green-800 mt-3 inline-block hover:underline">
                ...
                <div key={b.id as string} className="border border-gray-100 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    ...
                    <div className="text-xs text-gray-400 mt-1">
                      DKK {b.price_dkk as number}
                      {b.donates_to_charity && (
                        <span className="ml-2 inline-flex items-center gap-1 text-rose-500">
                          <HeartIcon className="w-3 h-3" /> doneret
                        </span>
                      )}
                    </div>
                    {b.created_at && (
                      <div className="text-xs text-gray-400 mt-0.5">
                        {new Date(b.created_at as string).toLocaleDateString('da-DK')}
                      </div>
                    )}
                  </div>
