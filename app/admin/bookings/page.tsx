...
export default function BookingsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      const { data, error: bookingError } = await supabase.from('bookings').select('id, starts_at, status, price_dkk, contribution_dkk, professional_payout_dkk, candidate_profile_id, professional_profile_id').order('starts_at', { ascending: false })
      if (!active) return
      if (bookingError) {
