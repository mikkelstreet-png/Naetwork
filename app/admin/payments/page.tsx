...
const REQUIREMENTS = [
  ...
  'Regnskabs- og skattemodel for den faste 10/20/70-fordeling er godkendt',
  ...
  return (
    <>
      ...
      <section className="mt-7 grid gap-6 lg:grid-cols-2">
        <div className="border border-gray-200 bg-white p-6"><p className="editorial-label">Valgt arkitektur</p><h2 className="mt-3 text-xl font-black text-gray-950">Stripe Connect · destination charge</h2><p className="mt-3 text-sm leading-relaxed text-gray-600">Én kandidat betaler for én professionel pr. session. Momsen skilles ud, hvorefter nettoprisen bogføres som 10% til Kræftens Bekæmpelse, 20% til Naetwork og 70% til den professionelle. Hvis den professionelle har valgt donation, bogføres egen 70%-andel også til Kræftens Bekæmpelse, og udbetalingen er nul. Naetwork håndterer Stripe-gebyrer, refunds og chargebacks inden for sin andel.</p><dl className="mt-5 border-t border-gray-200">{[['Stripe-nøgler', configuration.configured ? 'Sat' : 'Mangler'], ['Aktivering', configuration.enabled ? 'Aktiv' : 'Låst'], ['Valuta', 'DKK'], ['Checkout', 'Hosted Checkout']].map(([label, value]) => <div key={label} className="flex justify-between gap-4 border-b border-gray-200 py-3 text-sm"><dt className="text-gray-500">{label}</dt><dd className="font-bold text-gray-950">{value}</dd></div>)}</dl></div>
