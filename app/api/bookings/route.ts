...
import { copenhagenDateTimeToUtc, formatSessionDate } from '@/lib/dateTime';
...
export async function GET() {
  try {
    ...
    const { data: bookings, error } = await admin
      .from('bookings')
      .select('id, candidate_profile_id, professional_profile_id, starts_at, ends_at, status, price_dkk, message_to_professional, created_at')
      .or(filters.join(','))
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const startsAt = copenhagenDateTimeToUtc(preferredDate, preferredTime);
    if (!startsAt) return NextResponse.json({ error: 'Vælg et gyldigt tidspunkt.' }, { status: 400 });

    const now = Date.now();
    if (startsAt.getTime() < now + 2 * 60 * 60 * 1000 || startsAt.getTime() > now + 90 * 24 * 60 * 60 * 1000) {
      return NextResponse.json({ error: 'Tidspunktet skal være mindst 2 timer og højst 90 dage fremme.' }, { status: 400 });
    }
    ...
    const { data: booking, error: bookingError } = await admin
      ...
      .insert({
        ...
        price_dkk: professional.price_dkk,
