...
export async function POST(req: Request) {
  const { professionalEmail, professionalName, candidateName, sessionType, message, priceDkk } = await req.json();
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family: Inter, sans-serif; max-width: 560px; margin: 0 auto; padding: 40px 20px; color: #0a0a0a;">
        <h1 style="font-size: 24px; font-weight: 700; margin-bottom: 8px;">Du har en ny bookinganmodning</h1>
        <p style="color: #6b7280; margin-bottom: 24px;">${candidateName} ønsker en 60-minutters session med dig. Betaling håndteres separat efter bekræftelse.</p>
        <div style="border: 1px solid #f3f4f6; border-radius: 16px; padding: 20px; margin-bottom: 24px;">
          ...
          <div style="margin-bottom: 8px;"><strong>Sessionpris:</strong> ${formatDkk(priceDkk)}</div>
          ...
          ${message ? `<div style="margin-top: 12px; padding: 12px; background: #f9fafb; border-radius: 8px;"><strong>Brief:</strong><br/>${message}</div>` : ''}
        </div>
