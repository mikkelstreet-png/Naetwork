...
export default function BookingsPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      // Fetch bookings where user is candidate
      ...
      // Fetch bookings where user is professional
      ...
      if (proProfile) {
        const { data: pb } = await supabase
          ...
          .order('starts_at', { ascending: false })
        professionalBookings = pb ?? []
      }

      const all = [...(candidateBookings ?? []), ...professionalBookings]
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
        ...
        {loading ? (
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-10 text-center">
            ...
            <Link
              href="/professionals"
              className="inline-block px-5 py-2.5 bg-indigo-600 text-white text-sm font-semibold rounded-xl hover:bg-indigo-700 transition-colors"
            >
              {t.findPro}
            </Link>
          </div>
          ...
          <div className="space-y-3">
            {bookings.map((booking) => (
              <div
                key={booking.id}
                className="bg-white border border-gray-100 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3"
              >
                <div>
                  ...
                  {booking.price_dkk && (
                    <p className="text-xs text-gray-500 mt-1">{t.price} {booking.price_dkk}</p>
                  )}
                </div>
                <span
                  className={`self-start sm:self-auto px-3 py-1 rounded-full text-xs font-medium ${STATUS_STYLES[booking.status] ?? 'bg-gray-100 text-gray-600'}`}
                >
