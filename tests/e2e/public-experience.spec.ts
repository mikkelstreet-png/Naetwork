...
test('core public actions remain clear', async ({ page }) => {
  ...
  for (const amount of ['DKK 600', 'DKK 1.800', 'DKK 48', 'DKK 144']) await expect(page.getByText(amount, { exact: true }).first()).toBeVisible()
...
test('focused hero communicates insight and impact before format', async ({ page }) => {
  ...
  await expect(hero).toContainText('10 % af nettoprisen for hver gennemført og betalt session går til Kræftens Bekæmpelse.')
...
test('situation-first entry produces a relevant directory route', async ({ page }) => {
  ...
  const next = page.locator('#main-content').getByRole('button', { name: /Find relevant erfaring/i })
  await expect(next).toBeVisible()
  await next.click()
  await expect(page).toHaveURL(/\/professionals\?field=Consulting&session=industry-company-insight/)
})
