  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,#e8f4ef_0,#f7f8fb_38%,#f7f8fb_100%)] px-4 py-6 text-slate-950 sm:px-5">
      ...
      <section className="mx-auto grid max-w-6xl gap-6 py-10 lg:grid-cols-[.95fr_1.05fr] lg:py-20">
        <div>
          <p className="text-sm font-black uppercase tracking-[.22em] text-[#3f8f83]">Kunde-adgang</p>
          <h1 className="mt-4 text-5xl font-black leading-[.95] tracking-[-0.05em] text-[#071527] md:text-7xl">Se din opgave.</h1>
          <p className="mt-6 max-w-xl text-lg leading-8 text-slate-600">Få adgang til status, foreløbig brief, scope, spørgsmål og næste skridt. Ingen kodeord. Du får et sikkert link på email.</p>
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            {[["1", "Indtast email"], ["2", "Få sikkert link"], ["3", "Se din opgave"]].map(([no, text]) => (
        ...
        <div className="rounded-[32px] border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
          {!sent ? (
            <>
              <p className="text-sm font-black uppercase tracking-[.18em] text-[#3f8f83]">Login uden kodeord</p>
              <h2 className="mt-3 text-3xl font-black tracking-tight text-[#071527]">Send mig mit opgavelink</h2>
              <p className="mt-3 text-sm leading-6 text-slate-600">Brug den email, du sendte opgaven fra. Hvis vi finder en opgave, sender vi et sikkert link.</p>
              <label className="mt-6 grid gap-2 text-sm font-bold text-slate-700">
                Email
                <input value={email} onChange={(event) => setEmail(event.target.value)} placeholder="din@email.dk" className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-950 outline-none transition focus:border-[#3f8f83] focus:ring-4 focus:ring-[#3f8f83]/10" />
              </label>
              ...
              <p className="mt-4 text-xs leading-5 text-slate-500">Af sikkerhedshensyn fortæller vi ikke, om emailen findes i systemet.</p>
            </>
          ) : (
            <div className="rounded-[26px] bg-emerald-50 p-6">
              ...
              <p className="mt-3 text-sm leading-7 text-emerald-900">Hvis emailen matcher en opgave hos Naetwork, har vi sendt et sikkert link til “Min opgave”.</p>
