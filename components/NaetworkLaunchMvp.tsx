...
const initialIntake: Intake = {
  ...
  need: "Jeg skal bruge en digital løsning til min virksomhed, men jeg er usikker på scope, pris og hvilken specialist jeg skal vælge.",
  audience: "Små virksomheder, iværksættere og private med digitale opgaver.",
  budget: "Afklares",
  deadline: "Afklares"
};

const categoryConfig: Record<Category, { title: string; specialist: string; budget: string; tags: string[]; scope: string[]; questions: string[]; example: string }> = {
  "Jeg er ikke sikker": {
    ...
    scope: ["Afklaring af opgavetype og ønsket resultat", "Klar projektbrief med første version", "Fravalg og grænser for scope", "Anbefalet specialisttype", "Match-tags til relevante specialister"],
    ...
    example: "Jeg har en idé, men ved ikke om jeg skal bruge en hjemmeside, automation eller webapp."
  },
  Hjemmeside: {
    ...
    example: "Jeg skal bruge en professionel hjemmeside til min konsulentforretning."
  },
  "Webapp / MVP": {
    ...
    scope: ["Landing page og intake-flow", "Brief preview", "Specialist match preview", "Projektstatus og tilbudsflow", "Deploy-klar første version"],
    ...
    example: "Jeg vil bygge en simpel platform, men skal have styr på første version."
  },
  Dashboard: {
    ...
    example: "Vi har data i Excel og vil have et ledelsesvenligt overblik."
  },
  Automation: {
    ...
    example: "Jeg vil automatisere et manuelt flow mellem mail, Sheets og opfølgning."
  },
  "Pitch deck": {
    ...
    example: "Jeg skal bruge et skarpt deck til kunder, investorer eller partnere."
  }
};

const specialistMatches = [
  { name: "Webapp-specialist", type: "MVP & produktbyggeri", score: 96, price: "38.000 kr.", time: "5 uger", tags: ["Next.js", "Vercel", "UX", "MVP"], note: "Stærkt match på afgrænset MVP, produktstruktur og hurtigt launch-flow." },
  { name: "Automation-specialist", type: "Workflows & interne tools", score: 91, price: "29.000 kr.", time: "3 uger", tags: ["Make", "Sheets", "Admin", "Workflow"], note: "Relevant hvis opgaven handler om at reducere manuelt arbejde og forbinde eksisterende systemer." },
  { name: "Design- og launch-specialist", type: "Landing page & UI", score: 87, price: "45.000 kr.", time: "6 uger", tags: ["Brand", "Landing", "UI", "Launch"], note: "Godt match hvis første version skal føles skarp, troværdig og klar til markedet." }
];

const beforeAfter = [
  ["Uden Naetwork", "Jeg ved ikke, hvad jeg skal bede om", "Tilbud bliver svære at sammenligne", "Jeg risikerer at vælge forkert specialist"],
  ["Med Naetwork", "Mit behov bliver gjort konkret", "Jeg får scope, fravalg og acceptkriterier", "Jeg kan vælge mellem få relevante matches"]
];

const useCases: Category[] = ["Hjemmeside", "Webapp / MVP", "Dashboard", "Automation", "Pitch deck"];
const outcomes = ["Klar projektbrief", "Anbefalet specialisttype", "Scope og fravalg", "Spørgsmål der bør afklares", "Sammenlignelige tilbud"];
const qualitySignals = ["Ingen endeløs profil-børs", "Scope før tilbud", "Få relevante matches", "Mere tryg beslutning"];
...
function Button({ children, onClick, secondary = false, className = "", type = "button" }: { children: ReactNode; onClick?: () => void; secondary?: boolean; className?: string; type?: "button" | "submit" }) {
...
export function NaetworkLaunchMvp() {
  ...
  const [selectedSpecialist, setSelectedSpecialist] = useState(specialistMatches[0]);
  const [email, setEmail] = useState("");
  const [interestStatus, setInterestStatus] = useState<InterestStatus>("idle");
  const [providerStatus, setProviderStatus] = useState<InterestStatus>("idle");
  ...
  const brief = useMemo(() => ({
    ...
    acceptance: ["Kerneflow virker på desktop og mobil", "Leverancen matcher godkendt brief", "Specialisten afleverer kort handover", "Kunden kan godkende eller samle én feedbackrunde"]
  ...
  const submitInterest = () => setInterestStatus("sent");
  const submitProvider = () => setProviderStatus("sent");

  return <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,#e8f4ef_0,#f7f8fb_34%,#f7f8fb_100%)] text-slate-950">
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4">
        <button onClick={() => open("home")} className="flex items-center gap-3 text-left"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-[#071527] text-sm font-black text-white">N</span><span><span className="block text-lg font-black tracking-tight">Naetwork</span><span className="block text-xs text-slate-500">Early access</span></span></button>
    ...
    {view === "home" && <>
      <section className="mx-auto grid max-w-7xl items-center gap-8 px-5 py-10 lg:grid-cols-[1.04fr_.96fr] lg:py-20">
        <div>
          <div className="mb-5 inline-flex rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-600 shadow-sm">Early access · gratis at starte · uden binding</div>
          <h1 className="max-w-5xl text-4xl font-black leading-[.96] tracking-[-0.05em] text-[#071527] md:text-7xl">Beskriv din digitale opgave — få en klar brief og relevante specialister.</h1>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">Naetwork hjælper dig med at gøre en uklar idé konkret, så du kan sammenligne tilbud og vælge specialist med mere tryghed.</p>
        ...
        <Panel dark className="relative overflow-hidden lg:p-8">
          ...
          <div className="relative flex items-start justify-between gap-6"><div><p className="text-sm font-black uppercase tracking-[.2em] text-emerald-200">Det du får</p><h2 className="mt-3 text-3xl font-black tracking-tight">Fra idé til brief, før du vælger specialist</h2></div><Badge dark>Early access</Badge></div>
          ...
          <div className="relative mt-7 rounded-2xl bg-emerald-300/10 p-5 ring-1 ring-emerald-200/20"><p className="text-sm font-black text-emerald-100">AI bruges til at stille de rigtige spørgsmål og gøre dit behov klart. Matching og tilbud kører derefter struktureret.</p></div>
        </Panel>
      </section>

      <section className="mx-auto max-w-7xl px-5 py-8">
        <div className="grid gap-4 md:grid-cols-3">{[["1", "Beskriv med egne ord", "Du behøver ikke kende tekniske termer eller skrive en kravspecifikation."], ["2", "Få en skarp brief", "Naetwork gør behovet konkret med scope, fravalg og acceptkriterier."], ["3", "Vælg med mere tryghed", "Du får få relevante matches og tilbud, der kan sammenlignes." ]].map(([number, title, text]) => <Panel key={title} className="transition hover:-translate-y-1 hover:shadow-md"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-[#071527] text-sm font-black text-white">{number}</span><h3 className="mt-5 text-xl font-black text-[#071527]">{title}</h3><p className="mt-3 text-sm leading-6 text-slate-600">{text}</p></Panel>)}</div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 py-12">
        ...
        <div className="grid gap-4 md:grid-cols-5">{useCases.map((item) => <button key={item} onClick={() => { setIntake({ ...intake, category: item, need: categoryConfig[item].example }); open("brief"); }} className="rounded-[24px] border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-1 hover:border-[#3f8f83]/40 hover:shadow-md"><p className="font-black text-[#071527]">{item}</p><p className="mt-3 text-sm leading-6 text-slate-600">Få scope, specialisttype og relevant match-grundlag.</p></button>)}</div>
      </section>

      <section className="mx-auto max-w-7xl px-5 py-12"><Panel className="grid gap-6 lg:grid-cols-[.85fr_1.15fr]"><div><Eyebrow>Early access</Eyebrow><h2 className="mt-3 text-4xl font-black tracking-tight text-[#071527]">Vi åbner gradvist for de første opgaver.</h2><p className="mt-4 leading-7 text-slate-600">Formålet er at sikre høj kvalitet i briefs, matches og leverancer. Du kan starte med at få gjort din opgave konkret — uden binding.</p></div><div><div className="grid gap-3 md:grid-cols-2">{qualitySignals.map((item) => <div key={item} className="rounded-2xl bg-slate-50 p-4 text-sm font-black text-slate-700">{item}</div>)}</div><div className="mt-5 flex flex-col gap-3 rounded-2xl bg-slate-50 p-4 sm:flex-row"><input value={email} onChange={(event) => setEmail(event.target.value)} placeholder="Din email" className="min-w-0 flex-1 rounded-full border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-[#3f8f83] focus:ring-4 focus:ring-[#3f8f83]/10" /><Button onClick={submitInterest}>{interestStatus === "sent" ? "Du er skrevet op" : "Skriv mig op"}</Button></div></div></Panel></section>
    </>}

    {view === "brief" && <section className="mx-auto grid max-w-7xl gap-6 px-5 py-10 lg:grid-cols-[.82fr_1.18fr]">
      <Panel>
        <Eyebrow>Projektbrief</Eyebrow><h1 className="mt-3 text-4xl font-black tracking-tight text-[#071527]">Start med behovet — ikke kravspecifikationen.</h1><p className="mt-3 text-sm leading-6 text-slate-600">Få gjort opgaven konkret, før du skal vælge specialist eller sammenligne tilbud.</p>
        <div className="mt-6 grid gap-4">
          <div><p className="mb-2 text-sm font-bold text-slate-700">Hvad minder opgaven mest om?</p><div className="flex flex-wrap gap-2">{(Object.keys(categoryConfig) as Category[]).map((item) => <button key={item} onClick={() => setIntake({ ...intake, category: item, need: item === "Jeg er ikke sikker" ? intake.need : categoryConfig[item].example })} className={cx("rounded-full border px-4 py-2 text-sm font-black transition", intake.category === item ? "border-[#071527] bg-[#071527] text-white" : "border-slate-200 bg-white text-slate-700 hover:border-[#3f8f83]/50")}>{item}</button>)}</div></div>
          ...
          <Button onClick={() => open("matches")}>Se relevante specialisttyper</Button>
        </div>
      </Panel>
      <Panel>
        <div className="flex items-start justify-between gap-4 border-b border-slate-100 pb-5"><div><Eyebrow>AI-assisteret brief</Eyebrow><h2 className="mt-3 text-3xl font-black tracking-tight text-[#071527]">{brief.title}</h2></div><Badge>Early access</Badge></div>
        ...
        <div className="mt-6 grid gap-6 md:grid-cols-2"><div><h3 className="font-black">Scope</h3><ul className="mt-3 grid gap-2">{brief.scope.map((item) => <li key={item} className="rounded-2xl bg-slate-50 p-3 text-sm leading-6 text-slate-700">{item}</li>)}</ul><h3 className="mt-6 font-black">Spørgsmål der bør afklares</h3><ul className="mt-3 grid gap-2">{brief.questions.map((item) => <li key={item} className="rounded-2xl bg-emerald-50 p-3 text-sm leading-6 text-emerald-900">{item}</li>)}</ul></div><div><h3 className="font-black">Acceptkriterier</h3><ul className="mt-3 grid gap-2">{brief.acceptance.map((item) => <li key={item} className="rounded-2xl bg-slate-50 p-3 text-sm leading-6 text-slate-700">{item}</li>)}</ul><h3 className="mt-6 font-black">Ikke inkluderet</h3><ul className="mt-3 grid gap-2">{brief.notIncluded.map((item) => <li key={item} className="rounded-2xl bg-slate-50 p-3 text-sm leading-6 text-slate-700">{item}</li>)}</ul></div></div>
      </Panel>
    </section>}

    {view === "matches" && <section className="mx-auto max-w-7xl px-5 py-10"><div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><Eyebrow>Sådan virker matching</Eyebrow><h1 className="mt-3 text-4xl font-black tracking-tight text-[#071527]">Få relevante specialisttyper — ikke en endeløs profil-liste.</h1><p className="mt-3 max-w-3xl text-slate-600">Når briefen er klar, kan specialisttyper sammenlignes på match, prisniveau og leveringstid. Det gør valget mere trygt.</p></div><Button secondary onClick={() => open("brief")}>Tilbage til brief</Button></div><div className="grid gap-5 lg:grid-cols-3">{specialistMatches.map((specialist) => <Panel key={specialist.name} className="flex min-h-[430px] flex-col justify-between transition hover:-translate-y-1 hover:shadow-md"><div><div className="flex items-start justify-between gap-4"><div><p className="text-2xl font-black text-[#071527]">{specialist.name}</p><p className="mt-1 text-sm font-bold text-slate-500">{specialist.type}</p></div><span className="rounded-full bg-emerald-50 px-3 py-2 text-sm font-black text-emerald-800">{specialist.score}%</span></div><p className="mt-5 text-sm leading-6 text-slate-600">{specialist.note}</p><div className="mt-5 flex flex-wrap gap-2">{specialist.tags.map((tag) => <Badge key={tag}>{tag}</Badge>)}</div><div className="mt-6 grid grid-cols-2 gap-3"><Stat label="Prisniveau" value={specialist.price} /><Stat label="Tid" value={specialist.time} /></div></div><Button onClick={() => { setSelectedSpecialist(specialist); open("project"); }}>Vælg retning</Button></Panel>)}</div></section>}

    {view === "project" && <section className="mx-auto grid max-w-7xl gap-6 px-5 py-10 lg:grid-cols-[.85fr_1.15fr]"><Panel dark><Eyebrow>Valgt retning</Eyebrow><h1 className="mt-3 text-4xl font-black tracking-tight">{selectedSpecialist.name}</h1><p className="mt-4 text-white/70">{selectedSpecialist.note}</p><div className="mt-6 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl bg-white/10 p-4"><p className="text-sm text-white/60">Prisniveau</p><p className="mt-1 text-2xl font-black">{selectedSpecialist.price}</p></div><div className="rounded-2xl bg-white/10 p-4"><p className="text-sm text-white/60">Forventet tid</p><p className="mt-1 text-2xl font-black">{selectedSpecialist.time}</p></div></div><div className="mt-6 flex flex-col gap-3 sm:flex-row"><Button secondary onClick={() => open("how")}>Se næste trin</Button><Button secondary onClick={() => open("brief")}>Tilpas brief</Button></div></Panel><Panel><Eyebrow>Næste trin</Eyebrow><h2 className="mt-3 text-3xl font-black tracking-tight text-[#071527]">{brief.title}</h2><div className="mt-6 grid gap-3">{["Brief færdiggøres", "Relevante specialister udvælges", "Tilbud kan sammenlignes", "Specialist vælges", "Projekt startes", "Leverance og handover"].map((step, index) => <div key={step} className="flex items-center gap-4 rounded-2xl bg-slate-50 p-4"><span className={cx("grid h-9 w-9 shrink-0 place-items-center rounded-full text-xs font-black", index < 3 ? "bg-[#071527] text-white" : "bg-white text-slate-400 ring-1 ring-slate-200")}>{index + 1}</span><div><p className="font-black text-[#071527]">{step}</p><p className="text-sm text-slate-500">{index < 3 ? "Afklaret gennem Naetwork" : "Næste fase"}</p></div></div>)}</div></Panel></section>}

    {view === "provider" && <section className="mx-auto grid max-w-7xl gap-6 px-5 py-10 lg:grid-cols-[.85fr_1.15fr]"><Panel><Eyebrow>For specialister</Eyebrow><h1 className="mt-3 text-4xl font-black tracking-tight text-[#071527]">Bedre opgaver med klarere scope.</h1><p className="mt-4 text-slate-600">Specialister får værdi, fordi Naetwork sender opgaver med bedre brief, budget, forventninger og acceptkriterier.</p><div className="mt-6 grid gap-3">{["Mindre spildtid på uklare leads", "Brief og scope før tilbud", "Match-score pr. opgave", "Færre irrelevante henvendelser", "Mere professionel handover"].map((item) => <div key={item} className="rounded-2xl bg-slate-50 p-4 text-sm font-black text-slate-700">{item}</div>)}</div></Panel><Panel dark><h2 className="text-3xl font-black tracking-tight">Bliv en del af specialistnetværket</h2><p className="mt-4 leading-7 text-white/70">Naetwork åbner gradvist for specialister, så kvaliteten i matches og leverancer kan holdes høj.</p><div className="mt-6 grid gap-3">{["Kompetencer", "Opgavetyper", "Prisniveau", "Kapacitet", "Cases/links"].map((item) => <div key={item} className="rounded-2xl bg-white/10 p-4 text-sm font-black text-white/80 ring-1 ring-white/10">{item}</div>)}</div><div className="mt-6 flex flex-col gap-3 sm:flex-row"><Button secondary onClick={submitProvider}>{providerStatus === "sent" ? "Interesse registreret" : "Ansøg som specialist"}</Button><Button secondary onClick={() => open("brief")}>Se kunde-flow</Button></div></Panel></section>}
