...
const simpleSteps = [
  ["01", "Fortæl opgaven", "Skriv hvad du gerne vil have hjælp til. Du behøver ikke kende løsningen."],
  ["02", "Vi gør den klar", "Naetwork gør opgaven mere tydelig med scope, spørgsmål og næste skridt."],
  ["03", "Kom videre", "Du får en bedre specialistretning og vælger selv, om du vil fortsætte."]
];
...
export function NaetworkSprintFour() {
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,#e8f4ef_0,#f7f8fb_34%,#f7f8fb_100%)] text-slate-950">
      ...
      {view === "how" && <section className="mx-auto max-w-7xl px-5 py-12 lg:py-16">
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-3">{simpleSteps.map(([number, title, text]) => <Card key={number} className="text-center"><span className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-[#071527] text-sm font-black text-white">{number}</span><h2 className="mt-5 text-2xl font-black text-[#071527]">{title}</h2><p className="mt-3 text-sm leading-7 text-slate-600">{text}</p></Card>)}</div>
