...
export default function BookingDrawer({ professional, open, onClose, locale = 'da' }: BookingDrawerProps) {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      const focusLabel = FOCUS_OPTIONS.find((option) => option.id === sessionFocus)?.[locale] ?? sessionFocus
      const briefLabels = locale === 'da'
        ? { focus: 'Fokus', goal: 'Mål', material: 'Materiale/link', note: 'Note' }
        : { focus: 'Focus', goal: 'Goal', material: 'Material/link', note: 'Note' }
      const brief = [
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          ...
          {step === 2 && selectedDate && selectedTime && (
            <div>
              ...
              <div className="mb-5 flex items-center justify-between rounded-2xl border border-gray-200 bg-white px-4 py-3">
              ...
              <div className="mb-5">
                <label className="mb-3 block text-sm font-bold text-gray-950">{t.focusLabel}</label>
                  ...
                  {FOCUS_OPTIONS.map((option) => {
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-xl border px-3 py-2.5 text-left text-xs font-bold transition-colors ${selected ? 'border-gray-950 bg-gray-950 text-white' : 'border-gray-200 bg-gray-50 text-gray-700 hover:border-gray-950 hover:bg-white hover:text-gray-950'}`}
                      >
                        {label}
                      </button>
                    )
              ...
              <label className="mb-2 block text-sm font-bold text-gray-950">{t.goalLabel}</label>
              <textarea
                value={sessionGoal}
              ...
              <label className="mb-2 mt-4 block text-sm font-bold text-gray-950">{t.materialLabel}</label>
              <input
                value={materialLink}
                onChange={(e) => setMaterialLink(e.target.value.slice(0, 140))}
                placeholder={t.materialPlaceholder}
                inputMode="url"
                className="w-full rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-700 outline-none transition-colors focus:border-gray-950"
              />

              <label className="mb-2 mt-4 block text-sm font-bold text-gray-950">{t.noteLabel}</label>
              <textarea
                value={message}
