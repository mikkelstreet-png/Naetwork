...
const simpleSteps = [
  ...
  ["03", "Kom videre", "Du vælger selv, om opgaven skal sendes videre. Ingen binding på første trin."]
];
      ...
      <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4">
          <button type="button" onClick={() => open("home")} className="flex items-center gap-3 text-left">
            ...
            <span><span className="block text-lg font-black tracking-tight">Naetwork</span><span className="block text-xs text-slate-500">Få opgaven gjort klar</span></span>
          </button>
          <nav className="hidden items-center gap-2 lg:flex">
            {navItems.map(([target, label]) => <button key={target} type="button" onClick={() => open(target)} className={cn("rounded-full px-4 py-2 text-sm font-bold transition", view === target ? "bg-[#071527] text-white" : "text-slate-600 hover:bg-slate-100")}>{label}</button>)}
          </nav>
          <div className="flex items-center gap-2">
            <Button onClick={() => { setTaskStep(0); open("task"); }}>Opret opgave</Button>
            <button type="button" onClick={() => setMobileOpen((value) => !value)} className="rounded-full border border-slate-200 px-4 py-3 text-sm font-black text-slate-700 lg:hidden">Menu</button>
          </div>
        </div>
      ...
      {view === "home" && <>
        <section className="mx-auto grid max-w-7xl items-start gap-8 px-5 py-12 lg:grid-cols-[1.02fr_.98fr] lg:py-20">
          <div>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row"><Button onClick={() => { setTaskStep(0); open("task"); }}>Opret opgave</Button><Button secondary onClick={() => open("how")}>Se hvordan det virker</Button></div>
            <div className="mt-6 flex flex-wrap gap-2">{["Ingen betaling på første trin", "Du behøver ikke være teknisk", "Opgaven kan justeres senere"].map((item) => <span key={item} className="rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-black text-slate-700">{item}</span>)}</div>
          </div>
          <Card className="p-5 md:p-7">
            <div className="mb-4 flex items-start justify-between gap-4"><div><p className="text-lg font-black text-[#071527]">Så simpelt kan du starte</p><p className="mt-1 text-sm leading-6 text-slate-500">Du skriver ikke en kravspecifikation. Du beskriver bare opgaven, som du ser den.</p></div><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-800">Eksempel</span></div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5 text-sm leading-7 text-slate-700">{exampleNeed}</div>
            <div className="mt-5 rounded-2xl bg-[#071527] p-5 text-white"><p className="text-xs font-black uppercase tracking-[.18em] text-emerald-200">Naetwork gør det klarere</p><p className="mt-3 text-sm leading-6 text-white/75">Opgaven peger mod hjemmeside, kontaktflow, tekst og struktur. Den relevante specialistretning er sandsynligvis hjemmesidehjælp.</p></div>
          </Card>
        </section>

        <section className="mx-auto max-w-7xl px-5 py-10">
          <div className="mb-7 flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><Eyebrow>Opgaver Naetwork passer til</Eyebrow><h2 className="mt-3 text-4xl font-black tracking-tight text-[#071527]">De ting du ved, burde fungere bedre</h2><p className="mt-4 max-w-3xl text-sm leading-7 text-slate-600">Klik på en opgavetype for at starte med et mere relevant udgangspunkt.</p></div><Button secondary onClick={() => { setTaskStep(0); open("task"); }}>Start med egen opgave</Button></div>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{useCases.map((item) => <button key={item.title} type="button" onClick={() => chooseUseCase(item.category)} className="group rounded-[24px] border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-1 hover:border-[#3f8f83]/50 hover:shadow-md"><div className="mb-5 flex items-start justify-between gap-4"><span className="rounded-full bg-slate-50 px-3 py-1 text-xs font-black text-slate-500">{item.signal}</span><span className="text-lg font-black text-slate-300 transition group-hover:text-[#3f8f83]">→</span></div><p className="font-black text-[#071527]">{item.title}</p><p className="mt-3 text-sm leading-6 text-slate-600">{item.text}</p></button>)}</div>
        </section>

        <section className="mx-auto max-w-7xl px-5 py-10"><Card className="grid gap-6 lg:grid-cols-[.85fr_1.15fr]"><div><Eyebrow>Hvorfor det virker</Eyebrow><h2 className="mt-3 text-3xl font-black tracking-tight text-[#071527]">En bedre opgave giver et bedre match.</h2><p className="mt-4 text-sm leading-7 text-slate-600">Naetwork starter ikke med at vise profiler. Først gøres opgaven klarere, så det bliver nemmere at vælge den rigtige specialistretning.</p></div><div className="grid gap-4 md:grid-cols-3">{simpleSteps.map(([number, title, text]) => <div key={number} className="rounded-2xl bg-slate-50 p-4"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-[#071527] text-xs font-black text-white">{number}</span><p className="mt-4 font-black text-[#071527]">{title}</p><p className="mt-2 text-sm leading-6 text-slate-600">{text}</p></div>)}</div></Card></section>
      </>}

      {view === "how" && <section className="mx-auto max-w-7xl px-5 py-12 lg:py-16">
        <div className="mx-auto max-w-3xl text-center"><Eyebrow>Sådan virker det</Eyebrow><h1 className="mt-4 text-4xl font-black tracking-tight text-[#071527] md:text-6xl">Tre trin. Ikke mere kompliceret.</h1><p className="mt-5 text-lg leading-8 text-slate-600">Du beskriver opgaven. Naetwork gør den klarere. Du vælger selv, om du vil videre med en specialist.</p></div>
        <div className="mt-10 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr] md:items-stretch">{simpleSteps.map(([number, title, text], index) => <div key={number} className="contents"><Card className="text-center"><span className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-[#071527] text-sm font-black text-white">{number}</span><h2 className="mt-5 text-2xl font-black text-[#071527]">{title}</h2><p className="mt-3 text-sm leading-7 text-slate-600">{text}</p></Card>{index < 2 && <div className="hidden items-center px-2 text-3xl font-black text-slate-300 md:flex">→</div>}</div>)}</div>
        <div className="mt-8 grid gap-4 lg:grid-cols-[.9fr_1.1fr]"><Card><p className="text-xs font-black uppercase tracking-[.18em] text-slate-400">Før</p><p className="mt-3 text-2xl font-black text-[#071527]">“Jeg skal bruge hjælp til min hjemmeside.”</p><p className="mt-3 text-sm leading-6 text-slate-600">For bredt til at vælge den rigtige specialist.</p></Card><Card dark><p className="text-xs font-black uppercase tracking-[.18em] text-emerald-200">Efter Naetwork</p><p className="mt-3 text-2xl font-black">“Opgaven handler om kontaktflow, tekst og struktur, så flere besøgende sender en henvendelse.”</p><p className="mt-3 text-sm leading-6 text-white/70">Nu er opgaven lettere at forstå, vurdere og sende videre.</p></Card></div>
        <div className="mt-8 flex justify-center"><Button onClick={() => { setTaskStep(0); open("task"); }}>Opret opgave</Button></div>
      </section>}
