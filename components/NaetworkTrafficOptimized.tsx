...
type Intake = { category: Category; need: string; situation: string; outcome: string; audience: string; budget: string; deadline: string };
type Meta = { title: string; specialist: string; tags: string[]; scope: string[]; questions: string[] };
...
const categoryMeta: Record<Category, Meta> = {
  "Ikke sikker": { title: "Digital opgave gjort klar før du vælger specialist", specialist: "Digital produkt specialist", tags: ["Afklaring", "Brief", "Specialistretning"], scope: ["Afklare hvad der faktisk skal laves", "Skelne mellem første version og senere ønsker", "Finde mest relevant specialistretning"], questions: ["Hvad skal opgaven hjælpe dig med at opnå?", "Hvad er vigtigst i første version?", "Hvad kan vente?"] },
  "Hjemmeside": { title: "Hjemmeside der skaber flere relevante henvendelser", specialist: "Hjemmeside-specialist", tags: ["Hjemmeside", "Leads", "Kontaktflow"], scope: ["Forbedre struktur og vigtigste sider", "Gøre kontaktflow tydeligere", "Stramme tekst og call-to-actions op"], questions: ["Hvad skal besøgende gøre på siden?", "Hvor kommer trafikken fra?", "Hvilke henvendelser er mest værdifulde?"] },
  "Webapp / MVP": { title: "Første version af digital idé med klart kerneflow", specialist: "MVP-bygger", tags: ["MVP", "Webapp", "Launch"], scope: ["Definere kerneflow", "Prioritere første version", "Gøre løsningen klar til test"], questions: ["Hvad er vigtigste brugerflow?", "Hvad skal med i første version?", "Hvad kan vente?"] },
  "Dashboard": { title: "Dashboard med overblik over centrale nøgletal", specialist: "Data-specialist", tags: ["Dashboard", "Data", "Rapportering"], scope: ["Kortlægge datakilder", "Definere nøgletal", "Samle overblik i én visning"], questions: ["Hvor ligger data i dag?", "Hvilke tal styrer du efter?", "Hvem skal bruge dashboardet?"] },
  "Automation": { title: "Automation der fjerner manuelt dobbeltarbejde", specialist: "Automation-specialist", tags: ["Automation", "Workflow", "Proces"], scope: ["Kortlægge manuelt flow", "Opsætte simpelt workflow", "Teste og dokumentere brugen"], questions: ["Hvad starter flowet?", "Hvor skal data ende?", "Hvad sker der, hvis noget fejler?"] },
  "Pitch deck": { title: "Pitch eller salgsdeck med klar fortælling", specialist: "Præsentations-specialist", tags: ["Slides", "Storyline", "Design"], scope: ["Stramme storyline op", "Bygge slide-struktur", "Gøre materialet mødeklart"], questions: ["Hvem skal se materialet?", "Hvilken beslutning skal det drive?", "Har du tal og input klar?"] },
  "AI i virksomheden": { title: "Praktisk AI eller automation brugt rigtigt i virksomheden", specialist: "AI workflow-specialist", tags: ["AI", "Workflow", "Output"], scope: ["Finde relevante AI-brugsscenarier", "Afgrænse første praktiske workflow", "Gøre løsningen enkel at bruge"], questions: ["Hvor bruger du mest tid i dag?", "Hvilket output skal forbedres?", "Hvem skal bruge flowet?"] }
};

const useCases: Array<{ title: string; text: string; category: Category; signal: string; pain: string }> = [
  { title: "Hjemmeside der ikke skaber nok henvendelser", text: "Få gjort problemet klart, før du hyrer hjælp til design, tekst eller flow.", category: "Hjemmeside", signal: "Pejer ofte mod hjemmesidehjælp", pain: "Mere trafik end leads" },
  { title: "Manuelt arbejde i mails eller Excel", text: "Find ud af hvilke dele der kan automatiseres uden at bygge et stort system.", category: "Automation", signal: "Pejer ofte mod automation", pain: "For meget gentagelse" },
  { title: "Første version af en digital idé", text: "Få afgrænset første version, så idéen kan testes uden at blive for stor.", category: "Webapp / MVP", signal: "Pejer ofte mod MVP-bygger", pain: "God idé, uklart scope" },
  { title: "Dashboard og overblik", text: "Få defineret hvilke tal der betyder noget, og hvordan de skal samles.", category: "Dashboard", signal: "Pejer ofte mod datahjælp", pain: "Data ligger spredt" },
  { title: "Pitch deck eller kundemateriale", text: "Få gjort budskab og struktur skarpere, før materialet designes færdigt.", category: "Pitch deck", signal: "Pejer ofte mod præsentationshjælp", pain: "Budskabet sidder ikke" },
  { title: "Praktisk AI i virksomheden", text: "Find de steder, hvor AI faktisk sparer tid eller løfter output.", category: "AI i virksomheden", signal: "Pejer ofte mod AI-workflow", pain: "AI føles uklart" }
];
...
const providerTypes = ["Hjemmeside og frontend", "MVP og webapp", "Automation og workflows", "Dashboard og data", "Pitch decks og salgsflow", "Praktisk AI i virksomheder"];
...
function Card({ children, dark, className = "" }: { children: ReactNode; dark?: boolean; className?: string }) { return <div className={cn("rounded-[30px] border p-6 shadow-sm", dark ? "border-slate-800 bg-[#071527] text-white" : "border-slate-200 bg-white text-slate-950", className)}>{children}</div>; }
...
function TextArea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) { return <textarea {...props} className="w-full resize-none rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm leading-6 text-slate-950 outline-none focus:border-[#3f8f83] focus:ring-4 focus:ring-[#3f8f83]/10" />; }
function Input(props: React.InputHTMLAttributes<HTMLInputElement>) { return <input {...props} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-950 outline-none focus:border-[#3f8f83] focus:ring-4 focus:ring-[#3f8f83]/10" />; }

export function NaetworkTrafficOptimized() {
  ...
  const open = (next: View) => { setView(next); setMobileOpen(false); setTaskError(""); if (typeof window !== "undefined") window.scrollTo({ top: 0, behavior: "smooth" }); };
  const chooseUseCase = (category: Category) => { setIntake({ ...initialIntake, category, need: categoryMeta[category].title }); setTaskStep(1); open("task"); };
  ...
  const submitTask = async () => {
    ...
    try {
      ...
      setTaskId(result.id || ""); open("submitted");
    } catch (error) { setTaskError(error instanceof Error ? error.message : "Opgaven kunne ikke sendes lige nu."); } finally { setTaskLoading(false); }
  };
  ...
  const submitProvider = async () => {
    ...
    } catch (error) { setProviderError(error instanceof Error ? error.message : "Ansøgningen kunne ikke sendes lige nu."); } finally { setProviderLoading(false); }
  };

  const acceptCookies = () => { try { window.localStorage.setItem("naetwork_cookie_notice_accepted", "true"); } catch {} setCookieVisible(false); };
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,#e8f4ef_0,#f7f8fb_34%,#f7f8fb_100%)] text-slate-950">
      <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4">
          ...
          <div className="flex items-center gap-2"><Button onClick={() => { setTaskStep(0); open("task"); }}>Start kort</Button><button type="button" onClick={() => setMobileOpen((value) => !value)} className="rounded-full border border-slate-200 px-4 py-3 text-sm font-black text-slate-700 lg:hidden">Menu</button></div>
        </div>
      ...
      {view === "home" && <>
        <section className="mx-auto grid max-w-7xl items-start gap-8 px-5 py-12 lg:grid-cols-[1.02fr_.98fr] lg:py-20"><div><div className="mb-5 inline-flex rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-600 shadow-sm">For små virksomheder, founders og selvstændige</div><h1 className="max-w-5xl text-4xl font-black leading-[.96] tracking-[-0.05em] text-[#071527] md:text-7xl">Beskriv opgaven. Få den gjort klar. Kom videre med den rigtige specialist.</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">Naetwork hjælper dig, når du ved at noget digitalt bør fungere bedre, men ikke ved præcis hvem eller hvad du skal bestille.</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><Button onClick={() => { setTaskStep(0); open("task"); }}>Start med en kort beskrivelse</Button><Button secondary onClick={() => open("how")}>Se hvordan det virker</Button></div><div className="mt-6 flex flex-wrap gap-2">{["Ingen betaling på første trin", "Ingen teknisk kravspecifikation", "Du vælger selv næste skridt"].map((item) => <span key={item} className="rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-black text-slate-700">{item}</span>)}</div></div><Card className="p-5 md:p-7"><div className="mb-4 flex items-start justify-between gap-4"><div><p className="text-lg font-black text-[#071527]">Hvor starter de fleste?</p><p className="mt-1 text-sm leading-6 text-slate-500">Med en ufærdig tanke. Det er pointen.</p></div><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-800">Eksempel</span></div><div className="rounded-2xl border border-slate-200 bg-slate-50 p-5 text-sm leading-7 text-slate-700">{exampleNeed}</div><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-[#071527] p-4 text-white"><p className="text-xs font-black text-emerald-200">Behov</p><p className="mt-2 text-sm leading-6 text-white/75">Flere henvendelser</p></div><div className="rounded-2xl bg-[#071527] p-4 text-white"><p className="text-xs font-black text-emerald-200">Retning</p><p className="mt-2 text-sm leading-6 text-white/75">Hjemmesidehjælp</p></div><div className="rounded-2xl bg-[#071527] p-4 text-white"><p className="text-xs font-black text-emerald-200">Næste skridt</p><p className="mt-2 text-sm leading-6 text-white/75">Foreløbig brief</p></div></div></Card></section>

        <section className="mx-auto max-w-7xl px-5 py-6"><div className="grid gap-4 md:grid-cols-3"><Card><p className="font-black text-[#071527]">Jeg er klar</p><p className="mt-2 text-sm leading-6 text-slate-600">Jeg vil beskrive opgaven og få den gjort klar.</p><div className="mt-4"><Button full onClick={() => open("task")}>Opret opgave</Button></div></Card><Card><p className="font-black text-[#071527]">Jeg vil forstå først</p><p className="mt-2 text-sm leading-6 text-slate-600">Jeg vil se processen før jeg sender noget.</p><div className="mt-4"><Button secondary full onClick={() => open("how")}>Se processen</Button></div></Card><Card><p className="font-black text-[#071527]">Jeg er specialist</p><p className="mt-2 text-sm leading-6 text-slate-600">Jeg vil modtage bedre briefs og relevante opgaver.</p><div className="mt-4"><Button secondary full onClick={() => open("provider")}>For specialister</Button></div></Card></div></section>

        <section className="mx-auto max-w-7xl px-5 py-10"><div className="mb-7 flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><Eyebrow>Find dig selv her</Eyebrow><h2 className="mt-3 text-4xl font-black tracking-tight text-[#071527]">De ting du ved, burde fungere bedre</h2><p className="mt-4 max-w-3xl text-sm leading-7 text-slate-600">Jo hurtigere brugeren genkender sit problem, jo mere naturligt føles næste klik.</p></div><Button secondary onClick={() => { setTaskStep(0); open("task"); }}>Start med egen opgave</Button></div><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{useCases.map((item) => <button key={item.title} type="button" onClick={() => chooseUseCase(item.category)} className="group rounded-[24px] border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-1 hover:border-[#3f8f83]/50 hover:shadow-md"><div className="mb-5 flex items-start justify-between gap-4"><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-800">{item.pain}</span><span className="text-lg font-black text-slate-300 transition group-hover:text-[#3f8f83]">→</span></div><p className="font-black text-[#071527]">{item.title}</p><p className="mt-3 text-sm leading-6 text-slate-600">{item.text}</p><p className="mt-4 text-xs font-black text-slate-500">{item.signal}</p></button>)}</div></section>

        <section className="mx-auto max-w-7xl px-5 py-10"><Card className="grid gap-6 lg:grid-cols-[.75fr_1.25fr]"><div><Eyebrow>Hvorfor folk går videre</Eyebrow><h2 className="mt-3 text-3xl font-black tracking-tight text-[#071527]">Fordi opgaven føles mindre uoverskuelig.</h2><p className="mt-4 text-sm leading-7 text-slate-600">Naetwork gør første skridt lavt-friktions: brugeren sender ikke en perfekt kravspecifikation, men starter en afklaring.</p></div><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">{simpleSteps.map(([number, title, text]) => <div key={number} className="rounded-2xl bg-slate-50 p-4"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-[#071527] text-xs font-black text-white">{number}</span><p className="mt-4 font-black text-[#071527]">{title}</p><p className="mt-2 text-sm leading-6 text-slate-600">{text}</p></div>)}</div></Card></section>
      </>}

      {view === "how" && <section className="mx-auto max-w-7xl px-5 py-12 lg:py-16"><div className="mx-auto max-w-3xl text-center"><Eyebrow>Sådan virker det</Eyebrow><h1 className="mt-4 text-4xl font-black tracking-tight text-[#071527] md:text-6xl">Fire trin. Fra idé til udført arbejde.</h1><p className="mt-5 text-lg leading-8 text-slate-600">Du beskriver opgaven. Naetwork gør den klarere. Du vælger næste skridt. Specialisten udfører arbejdet direkte med dig.</p></div><div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] lg:items-stretch">{simpleSteps.map(([number, title, text], index) => <div key={number} className="contents"><Card className="text-center"><span className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-[#071527] text-sm font-black text-white">{number}</span><h2 className="mt-5 text-2xl font-black text-[#071527]">{title}</h2><p className="mt-3 text-sm leading-7 text-slate-600">{text}</p></Card>{index < simpleSteps.length - 1 && <div className="hidden items-center px-2 text-3xl font-black text-slate-300 lg:flex">→</div>}</div>)}</div><div className="mt-8 grid gap-4 lg:grid-cols-[.9fr_1.1fr]"><Card><p className="text-xs font-black uppercase tracking-[.18em] text-slate-400">Før</p><p className="mt-3 text-2xl font-black text-[#071527]">“Jeg skal bruge hjælp til min hjemmeside.”</p><p className="mt-3 text-sm leading-6 text-slate-600">For bredt til at vælge den rigtige specialist.</p></Card><Card dark><p className="text-xs font-black uppercase tracking-[.18em] text-emerald-200">Efter Naetwork</p><p className="mt-3 text-2xl font-black">“Opgaven handler om kontaktflow, tekst og struktur. Når pris og rammer er aftalt, udfører specialisten arbejdet.”</p><p className="mt-3 text-sm leading-6 text-white/70">Nu er opgaven lettere at forstå, vurdere og udføre.</p></Card></div><div className="mt-8 flex justify-center"><Button onClick={() => { setTaskStep(0); open("task"); }}>Start med kort beskrivelse</Button></div></section>}
